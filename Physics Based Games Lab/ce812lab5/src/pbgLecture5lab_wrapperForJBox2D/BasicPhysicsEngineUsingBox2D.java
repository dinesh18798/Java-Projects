...
public class BasicPhysicsEngineUsingBox2D {
	...
	private static BasicMouseListener basicMouseListener;
	private static BasicKeyListener keyListener;

	protected int noOfShoot = 0;
	...
	public CannonBallLauncher launcher;
	public static MouseJoint mouseJointDef;

	public AnchoredBarrier_StraightLine flyingFloor;
	public AnchoredBarrier_StraightLine flyingFloorOpposite;
...
//			public BasicRectangle(double sx, double sy, double vx, double vy, double width, double height, double orientation, double angularVeloctiy, boolean improvedEuler, Color col, double mass) {
		...
		launcher = new CannonBallLauncher(0, WORLD_HEIGHT/2,0f,0f,0.5f, Color.CYAN, 10, 0f);
...
//		
		...
		if (true) {
			...
			flyingFloor = new AnchoredBarrier_StraightLine(WORLD_WIDTH / 2 - 2, WORLD_HEIGHT / 4f, WORLD_WIDTH / 2 + 2, WORLD_HEIGHT / 4, Color.CYAN);
			flyingFloorOpposite = new AnchoredBarrier_StraightLine(WORLD_WIDTH / 2 + 2, WORLD_HEIGHT / 2, WORLD_WIDTH / 2 - 2, WORLD_HEIGHT / 2, Color.RED);

			int counter = 4;
			float incrX = flyingFloor.body.getPosition().x;
			float incrY = flyingFloor.body.getPosition().y + 0.1f;
			float posX = incrX + flyingFloor.body.getWorldCenter().x;

			//Generate the projectiles that the player will try to throw down
			for (float i = incrY; i < 12; i++) {
				for (int j = 0; j < counter; j++) {
					polygons.add(new BasicPolygon(WORLD_WIDTH - posX, i, s, s, r * 1, Color.GREEN, 5, rollingFriction, 6));
					posX -= 0.1;
				}
				counter--;
			}
			...
			/*counter = 4;
			posX = incrX + flyingFloorOpposite.body.getWorldCenter().x;
			//Generate the projectiles that the player will try to throw down
			for (float i = incrY; i < 12; i++) {
				for (int j = 0; j < counter; j++) {
					polygons.add(new BasicPolygon(WORLD_WIDTH - posX, i, s, s, r * 1, Color.MAGENTA, 5, rollingFriction, 6));
					posX -= 0.1;
				}
				counter--;
			}*/
...
//				particles.add(new ParticleAttachedToMousePointer(WORLD_WIDTH/2,WORLD_HEIGHT/2,0,0, r/2, true, 10000));				
	...
	public void update() {
		...
		if(BasicKeyListener.isRotateUpKeyPressed()) {
			//launcher.bodySquare.getPosition();
			if( launcher.getRotation() < 0.6f)
				launcher.setRotation( launcher.getRotation() +  (float) Math.toRadians(1));
		}
		if(BasicKeyListener.isRotateDownKeyPressed())
		{
			if( launcher.getRotation() > -0.6f)
				launcher.setRotation( launcher.getRotation() -  (float) Math.toRadians(1));
		}
	...
	public void shootCanon() {
		Vec2 pos = new Vec2(launcher.bodySquare.getWorldCenter());
		if (pos2 == null){
			pos2 = new Vec2(launcher.bodySquare.getWorldCenter());
		}

		System.out.println("Insis Shoot");

		float canonRadius = launcher.getRadius();

		float ballRadius = 0.2f;
		//testPo();

		pos2.x = pos2.x * (float) Math.sin(launcher.bodySquare.getAngle());
		//pos2.y = pos2.y * (float) (Math.cos(cannon.body.getAngle()));

		Vec2 velocity = hitTheBall().mul(0.5f);
		destroyBalls();
		if (basicMouseListener.getWorldCoordinatesOfMousePointer().y < launcher.bodySquare.getPosition().y){
			System.out.println("Shoot");
			particles.add(new BasicParticle(pos2.x+canonRadius,pos2.y-canonRadius*2,velocity.x,velocity.y, ballRadius, Color.ORANGE, 1, 0));
		}
		else if (basicMouseListener.getWorldCoordinatesOfMousePointer().y > launcher.bodySquare.getPosition().y){

			System.out.println("Shoot");
			//cannon.getMainVertice(0);
			particles.add(new BasicParticle(pos2.x+canonRadius,pos2.y+canonRadius*2,velocity.x,velocity.y, ballRadius, Color.ORANGE, 1, 0));
		}
	}

	public Vec2 hitTheBall()
	{
		//System.out.println("Vertice: " + cannon.getMainVertice(2));

		Vec2 mousePointer = basicMouseListener.getWorldCoordinatesOfMousePointer();
		Vec2 can = launcher.bodySquare.getWorldCenter();
		Vec2 diff = mousePointer.sub(can);
		float finalOff = diff.length();
		Vec2 vel = diff.mul(finalOff);
		return vel;
	}
